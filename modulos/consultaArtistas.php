<?php
	try{
		...
		$sql=$conn->prepare('SELECT imagenes.imagen, artistas.nombreArtista, artistas.idArtista FROM relacionimagenes INNER JOIN imagenes ON imagenes.idImagen = relacionimagenes.idImagen INNER JOIN artistas ON artistas.idArtista = relacionimagenes.idArtista WHERE relacionimagenes.tipoImagen="principal" AND artistas.estado = "activo" ORDER BY artistas.idArtista ASC');
